...
export const insertTodoData = async (req, res) => {
	try {
		...
		await newTodo.save();
		res.json({ success: true, message: "New Todo Created SuccessFully" });
	} catch (err) {
...
export const deleteParticularTodos = async (req, res) => {
	try {
		...
		if (!findTodo) {
			return res.json({ success: false, message: "Specified Id related Todo not Found " });
		}
		...
		res.json({
			success: true,
			message: "Deletion of the specified todo was successfully",
			deletedTodo: todo,
		});
	} catch (err) {
...
export const updateTodoList = async (req, res) => {
	try {
		...
		if (isCompleted !== findTodo.isCompleted) {
			...
			res.json({ success: true, message: "Update Task Detail Successfully" });
		}
